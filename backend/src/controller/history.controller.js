...
export const getRequestHistoryController = async (req, res) =>{
    try{
        ...
        const {data, error} = await supabase
            ...
            .select("*, student_profiles(id_number, first_name, last_name, email, contact_number, program, users_public(email))")
            ...
            student_profiles: entry?.student_profiles
                ? {
                    ...entry.student_profiles,
                                        email: entry.student_profiles?.users_public?.email ?? entry.student_profiles?.email ?? null,
                  }
                : null,
        }));
